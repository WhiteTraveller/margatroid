global.spawnMob = function (level, x, y, z) {
    ...
    for (var j = 0; j < players.size(); j++) {
        ...
        if (AABB.of(x + 100, y + 100, z + 100, x - 100, y - 100, z - 100).contains([px, py, pz])) {
            for (let i = 0; i < (mobs.length); i++) {
                 ...
                 */
                for (let k = 0; k < mobs[i].count; k++) {
                    let mob = level.createEntity(mobs[i].customId);
                    mob.mergeNbt({ "PersistenceRequired": 1 });
                    mob.addTag("dungeon_mob");
                    mob.setPosition(x, y + 1, z);
                    setLevelWithReroll(mob, mobs[i].level, true);
                    mob.spawn()
                    let name = mob.getUuid().toString();
                    console.log(`Spawned mob: ${name}`);
                    if (mobs[i].trait) {
                        for (let trait of mobs[i].trait) {
                            server.runCommand(`hostility mobs ${name} trait set ${trait.traitId} ${trait.level}`);
                        }
                    }
                }
            }
 ...
 * @param {Internal.Entity} entity - the mob you want to check.
