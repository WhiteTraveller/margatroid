global.spawnMob = function (level, x, y, z, item) {
    let mobPool = global.mobPool.l1;
    let mobGroup = global.getRandomItemFromArray(mobPool.groups);
    let mobs = mobGroup.mobs;
    for (let i = 0; i < (mobs.length); i++) {
        for (let j = 0; j < mobs[i].count * ((item.nbt.getInt("level") + 1) || 1); j++) {
            let mob = level.createEntity(mobs[i].customId);
            mob.mergeNbt(mobs[i].nbt);
            mob.addTag(mobs[i].tag);
            mob.setPosition(x + 8, y + 5, z + 8);
            setLevelWithReroll(mob, mobs[i].level, true);
            if (mobs[i].trait) {
                for (let trait of mobs[i].trait) {
                    setTrait(mob, trait.trait, trait.level);
                }
            }
            mob.spawn();
        }
